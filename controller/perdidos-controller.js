...
function renderizarPerdidos(req, res) {
    renderizarEspecie(res, "cachorro");
}

function cadastrarPerdido(req, res){
    if (req.files["foto"] == undefined) {
        console.log("Não é uma foto");
    } else {
        salvarFoto(req.files["foto"].pop(), (caminho) => {
            if(caminho != null) {
                const perdido = new modeloPerdido.perfilPerdido(req.session.usuario.email,
                                    ...
                                    req.fields.data.pop(),
                                    caminho);
                modeloPerdido.PerdidoDAO.inserir(perdido, (resp) => {
                    if(resp === false){
                        res.render("perfil", {usuario: req.session.usuario, registrar:true, falha:true});
                    }else{
                        matchCotroller.matchingPerdido(perdido);
...
function renderizarEspecie(res, especie) {
    ...
    modeloPerdido.PerdidoDAO.buscar(query, (animais) => {
        vetoresUnicos(animais);
        let cachorroSelecionado = especie == "cachorro";
        res.render("perdidos", {cachorroSelecionado: cachorroSelecionado,
                                animais: animais,
                                raca: raca,
                                cor: cor,
                                porte: porte});
...
function filtrarPerdidos(req, res) {
    ...
    modeloPerdido.PerdidoDAO.buscar(query,(animais) => {
        ...
        vetoresUnicos(animais);
        res.render("perdidos", {cachorroSelecionado: valorEspecie == "cachorro",
                                animais: newAnimais,
                                raca: raca,
                                cor: cor,
                                porte: porte});
...
module.exports ={
    renderizarPerdidos : renderizarPerdidos,
    filtrarPerdidos: filtrarPerdidos,
    cadastrarPerdido: cadastrarPerdido,
